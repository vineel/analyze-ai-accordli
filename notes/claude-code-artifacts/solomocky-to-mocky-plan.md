...
**In scope:**
- **docker-compose for Postgres.** Replace `brew install postgresql@16` instructions with a docker-compose service pinned to the Postgres major version we'll run in Azure. Reproducible across machines, matches prod, doesn't fight whatever else lives on the laptop.
- **Per-developer Cloudflare Tunnel.** Each of us gets a stable subdomain (e.g., `vineel.dev.accordli.com`, `tom.dev.accordli.com`) pointing to localhost via `cloudflared`. One DNS record per developer, persistent across tunnel restarts, no expiring URLs. `make tunnel` brings it up. Document the one-time setup in `README.md`.
...
- **Per-developer external accounts** in `.env.example`: WorkOS staging environment (one per dev — keeps webhook streams clean), Helicone dev key, PostHog dev project, Postmark sandbox, Stripe staging key. Document which are shared and which are per-developer.
...
**In scope:**
- OpenTelemetry SDK in the Go binary. **Local exporter:** OTLP to a local OTel collector (docker-compose service) that writes to stdout or forwards to a free-tier hosted backend (Honeycomb / Grafana Cloud — Tom and Vineel pick one). The Azure-native exporter (App Insights) is added in Phase 8 alongside the prod exporter config.
...
- The data model in `db/migrations/0001`–`0004`. New tables get added; existing columns don't get renamed.
...
2. **WorkOS staging environments — per-developer or shared?** Per-developer is cleaner; shared is cheaper. Recommendation: per-developer until we have more than 4 of us.
3. **Local OTel backend.** Honeycomb vs Grafana Cloud vs stdout-only. Recommendation: stdout-only in Phase 6, pick a hosted backend when it actually hurts.
4. **PostHog event catalog owner.** Must be written before Phase 6 ships. Recommendation: Tom drafts, Vineel reviews.
5. **Where the summary call lives.** Currently on `review_runs.summary`, not as a Finding. `starter-app.md` §9.3 recommendation: keep on Run. Confirm before any phase that touches the summary path (none of 1–8 does in scope, but worth nailing).
6. **Postgres major version pin.** Whatever we put in docker-compose at Phase 0 is what we'll provision in Azure at Phase 8. Confirm: 16.
