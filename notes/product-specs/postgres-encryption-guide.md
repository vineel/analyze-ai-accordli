...
Most production systems run both. **We run only storage-layer encryption at v1, using a customer-managed key held in Accordli's Azure Key Vault.** Application-layer envelope encryption is on the deferred roadmap; the concept is documented here because we'll add it when a customer pulls it in.
...
| **CMK** — customer-managed key | Encryption key for database storage, held by *us* in our Azure Key Vault and used by Azure Postgres for storage encryption. The "customer" here is Azure's customer (Accordli), not the law firm. |
...
| **Azure Key Vault** | Azure's hardened key-management service. Standard tier is software-protected; Managed HSM is FIPS 140-3 hardware-backed. We use Standard for the v1 Postgres CMK. |
...
- **Lost or exfiltrated backup blob.** Backup files (or their copies) end up somewhere unintended. Anyone reading them sees AES-256 ciphertext at the storage layer; the key is held in Accordli's Azure Key Vault.
- **Tampered or intercepted DB traffic.** All connections to Postgres require TLS 1.3.
- **Loss of Microsoft-managed storage-key control.** The Postgres storage encryption key is customer-managed by Accordli, not service-managed by Microsoft.

What v1 does **not** defend against (be honest about this on questionnaires):
- **Plaintext reads through valid database access.** CMK is storage-layer encryption. Postgres still decrypts pages before evaluating queries, so a valid database connection with sufficient privileges can read plaintext rows.
...
- **Cryptographic tenant isolation.** Storage-layer CMK uses database/server-level encryption, not per-Organization keys. Org isolation remains logical at v1.
...
### 1. Storage-layer encryption: CMK on Flex Server

Provision Flex Server with a customer-managed key stored in Accordli's Azure Key Vault. AES-256 storage-layer encryption applies to all databases, logs, WAL segments, and backups.

The Key Vault and key must live in the same Azure region as the Postgres Flexible Server. The Postgres server's managed identity is granted the minimum Key Vault permissions required to unwrap the key. Deleting, disabling, expiring, or blocking access to the key can make the database server transition to `Inaccessible`; this is an accepted v1 operational dependency.

CMK is still storage-layer encryption. Postgres operates on plaintext internally after the storage layer decrypts pages, so normal queries, indexes, joins, JSONB operators, and ad hoc tools such as DataGrip continue to work for users with valid database access and sufficient privileges.

Key rotation is handled by runbook. Rotation changes the storage-layer key material used by Azure Postgres; it does not require re-encrypting application rows or changing query behavior.
...
- **Key Vault access is RBAC-gated and monitored.** The Postgres managed identity receives only the permissions required for the CMK. Human access to the key is break-glass only, named, and audited.

These controls are what the SOC 2 auditor will look for in the absence of column-level encryption. CMK improves the storage-layer key-control story, but these controls still carry the risk that a valid database connection can read plaintext.

### 4. What we explicitly do **not** do at v1

- **No application-layer envelope encryption.** Deferred to v2. Sensitive content sits in Postgres as ordinary types (`text`, `jsonb`, `bytea`), protected by storage-layer CMK and database access controls.
- **No per-Organization application keys.** The v1 CMK is database/server-level storage encryption, not tenant-isolated encryption.
- **No Managed HSM.** Standard Key Vault is used for the v1 Postgres CMK and non-encryption secrets (DB connection strings, API keys, third-party credentials).
...
- **Credentials.** Postgres credentials and Key Vault access are issued to managed identities, not service principals with stored secrets. No shared developer accounts on prod.
- **CMK availability.** The Key Vault, key, managed identity, access policy/RBAC assignment, and network path are production dependencies for database availability. Alerts must cover disabled/expired keys, Key Vault availability, and denied key access.
...
> All data is encrypted at rest using AES-256 in Azure Database for PostgreSQL Flexible Server, with a customer-managed key held in Accordli-controlled Azure Key Vault. All client connections are required to use TLS 1.3.
>
> Application-layer encryption with customer-isolated keys and customer-controlled root keys (HYOK) are on our roadmap.

What **not** to say at v1:
- Do not claim "application-level encryption" — CMK is storage-layer encryption.
